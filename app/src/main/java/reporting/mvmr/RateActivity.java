...
import reporting.mvmr.models.RatingsModel;
...
public class RateActivity extends AppCompatActivity implements RatingFragment.OnFragmentInteractionListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        stats = (EditText) findViewById(R.id.statInput);
        ...
        stats.setText(ratingsCache.getString("stats", null));
    ...
    protected void onPause() {
        ...
        if(interfaceC.getText() != null)
            ratingsCacheEditor.putString("interface", interfaceC.getText().toString()).commit();
        if(access.getText() != null)
            ratingsCacheEditor.putString("access", access.getText().toString()).commit();
        if(report.getText() != null)
            ratingsCacheEditor.putString("report", report.getText().toString()).commit();
        if(howto.getText() != null)
            ratingsCacheEditor.putString("howto", howto.getText().toString()).commit();
        if(about.getText() != null)
            ratingsCacheEditor.putString("about", about.getText().toString()).commit();
        if(general.getText() != null)
            ratingsCacheEditor.putString("general", general.getText().toString()).commit();
        if(stats.getText() != null)
            ratingsCacheEditor.putString("stats", stats.getText().toString()).commit();

    }

    public void onSubmit(View view) {
        //SharedPreferences settings = getSharedPreferences("MVMR", 0);
        TrayModule tray = new TrayModule(RateActivity.this);
        ...
        model.Statistics = stats.getText().toString();
        model.Ratings = ratings;
        model.User = tray.getString("user_id", null);
        SendResults(model);
    }
